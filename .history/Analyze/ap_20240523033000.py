...
def display_charts(relevance_percentage):
    ...
    explode = (0.1, 0)
    pie_ax.pie(sizes, explode=explode, labels=labels, colors=colors, autopct='%1.1f%%', shadow=True, startangle=140, textprops={'color': text_color})
    pie_ax.axis('equal')
    ...
    bar_ax.set_title('Relevance between File 1 and File 2', color=text_color)
    bar_ax.tick_params(axis='x', colors=text_color)
    bar_ax.tick_params(axis='y', colors=text_color)
    for i, v in enumerate(sizes):
        bar_ax.text(i, v + 2, f"{v:.1f}%", ha='center', va='bottom', color=text_color)
...
analyze_button = ttk.Button(frame, text="Analyze Data", command=analyze_data, style='Light.TButton')
analyze_button.grid(row=2, column=0, columnspan=3, pady=20)

relevance_label = ttk.Label(frame, text="", background="#FFFFFF", foreground="#000000")
relevance_label.grid(row=3, column=0, columnspan=3, pady=(5, 20))

chart_frame = ttk.Frame(frame, style='Light.TFrame')
chart_frame.grid(row=4, column=0, columnspan=3, pady=10, padx=10, sticky=tk.NSEW)

dark_mode_toggle = ttk.Checkbutton(frame, text="Dark Mode", variable=dark_mode, command=toggle_dark_mode)
dark_mode_toggle.grid(row=5, column=0, columnspan=3, pady=(10, 20))
